...
class HourlyDrop(commands.Cog):
    ...
    async def drop(self):
        if self.bot.is_ready():
            ...
            print(time_since_start.total_seconds() / 3600)
            channel = self.bot.get_channel(1485685766033117245 if self.bot.is_nougat else TEST_CHANNEL)
            await channel.send(f"Hour {hour} of posting Drop every hour until Escape from Wormwood comes out", file=discord.File(f"drop/{hour}.png"))
